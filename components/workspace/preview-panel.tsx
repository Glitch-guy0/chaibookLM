...
export function PreviewPanel({
  ...
  const contextBefore = "Retrieved passage context from original document source:";
  const contextAfter = "End of retrieved citation passage excerpt.";

  return (
    <aside className="w-96 shrink-0 border-l border-[hsl(215_25%_22%/0.6)] flex flex-col h-full animate-slide-in-right shadow-2xl z-20 bg-[hsl(var(--bg-h)_var(--bg-s)_var(--bg-l))]">
      {/* Panel Header */}
      <div className="p-4 border-b border-[hsl(215_25%_22%/0.6)] flex items-center justify-between shrink-0">
        ...
        <button
          type="button"
          onClick={onClose}
          className="p-1 rounded-lg text-text-muted hover:text-text hover:bg-[hsl(var(--surface-h)_var(--surface-s)_var(--surface-l))] transition-colors shrink-0 cursor-pointer"
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="rounded-xl border border-[hsl(215_25%_22%/0.5)] overflow-hidden bg-[hsl(var(--surface-h)_var(--surface-s)_var(--surface-l)/0.4)]">
              ...
              <div className="p-4 space-y-3 text-body-sm leading-relaxed font-body">
                ...
                {snippet && (
                  <p className="text-text-muted/70 italic border-l-2 border-transparent pl-3 text-xs">
                    {contextBefore}
                  </p>
                )}
                ...
                {snippet ? (
                  <div className="relative">
                    {/* Left accent bar */}
                    <div className="absolute left-0 top-0 bottom-0 w-1 rounded-full bg-gradient-to-b from-primary via-accent to-secondary" />
                    <div className="pl-4 pr-3 py-3 rounded-r-xl bg-[hsl(var(--accent-h)_var(--accent-s)_var(--accent-l)/0.08)] border border-[hsl(var(--accent-h)_var(--accent-s)_var(--accent-l)/0.25)] shadow-sm">
                      {/* Citation label */}
                      ...
                      <div className="text-text leading-relaxed font-body">
