...
/**
 * COLOR SYSTEM — do not add hex values here.
 * Every role below reads its h/s/l from CSS custom properties defined in
 * globals.css (once for :root/light, once for .dark). `tint` and `shade`
 * are computed live with calc(), so they always track the base value —
 * change the base once in globals.css and tint/shade follow automatically.
 */
    ...
    extend: {
      colors: {
        ...
        surface: {
          ...
          // subtle raised variant for nested cards inside panels
