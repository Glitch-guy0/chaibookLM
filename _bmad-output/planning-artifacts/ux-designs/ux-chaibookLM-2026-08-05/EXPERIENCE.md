...
| Ingestion | Source card | `queued → processing → ready/failed`, surfaced as status dot + label on the card (color per DESIGN.md). While indexing, the card's hard offset shadow animates as a **shadow-only rotating activity indicator** — the card body stays static (DESIGN.md Elevation & Depth). On `ready` the shadow snaps back to normal and the card **glows green for 1 second** then fades. Source is **not** queryable until `ready`. One polite live region announces only final states — `ready`/`failed` including the failure reason; intermediate `queued → processing` is not announced. |
