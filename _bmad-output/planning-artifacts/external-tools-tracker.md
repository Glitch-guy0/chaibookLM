...
| 9 | **Inngest** | Ingestion workflow engine & durable failure management (replaces QStash) | Yes — free tier | `INNGEST_EVENT_KEY`, `INNGEST_SIGNING_KEY` | **Configured (dev)** | Prajwal | Replaces QStash. Decomposes multi-modal ingestion into idempotent serverless step-functions with 3x retries, failure isolation, and per-user concurrency limits. |
| 10 | **LangChain** (`@langchain/core`, `@langchain/openai`, `langchain`) | RAG runtime, prompt templates, and streaming retrieval sequences (replaces shikigami) | No account — npm package | n/a | **Configured (dev)** | Prajwal | Replaces @glitch-guy0/shikigami. Completely decoupled behind standard ports-and-adapters; standardizes prompt templates and streaming runnable pipelines with no proprietary tight-coupling. |
| 11 | **Driver.js** | First-run product tour (FR-24) | No account — npm package | n/a | Configured (dev) | Prajwal | No tracking needed beyond install |
